...
async def create_test_accounts():
    ...
    async with AsyncSessionLocal() as db:
        ...
        # 2. Setup Test Agency
        result_agency = await db.execute(select(User).options(selectinload(User.agency_profile)).where(User.phone == agency_phone))
        ...
        print(f"   Name:  {agency_user.agency_profile.display_name if agency_user.agency_profile else agency_user.name}")
