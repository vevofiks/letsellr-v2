...
from app.modules.users.schemas import UserProfileResponse, UserUpdateRequest, VerificationSubmitRequest, ChangePinRequest
    ...
    if payload.agency_display_name is not None and current_user.role == "agency":
        if current_user.agency_profile:
            ...
            new_profile = AgencyProfile(
                ...
                display_name=payload.agency_display_name
            )
...
async def change_pin(
    ...
    if not current_user.auth_provider_uid or not verify_password(payload.old_pin, current_user.auth_provider_uid):
