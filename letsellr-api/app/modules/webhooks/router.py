...
@router.post("/crm/properties", response_model=PropertyResponse, status_code=status.HTTP_201_CREATED, dependencies=[Depends(verify_crm_secret)])
...
@router.get("/crm/properties/{property_ref}", response_model=PropertyResponse, dependencies=[Depends(verify_crm_secret)])
async def crm_get_property(property_ref: str, db: DbSession):
    service = PropertyService(db)
    return await service.get_property(property_ref)


@router.patch("/crm/properties/{property_ref}", response_model=PropertyResponse, dependencies=[Depends(verify_crm_secret)])
async def crm_update_property(property_ref: str, data: PropertyUpdate, db: DbSession):
    ...
    admin_user = User(id=prop.owner_id, role="admin", phone="0000000000", name="Admin", location_city="", location_area="", preference_type="")
    return await service.update_property(prop.id, data, admin_user)


@router.delete("/crm/properties/{property_ref}", status_code=status.HTTP_204_NO_CONTENT, dependencies=[Depends(verify_crm_secret)])
