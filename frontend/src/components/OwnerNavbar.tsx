        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-16 flex items-center justify-between gap-2">
          ...
          <div className="flex items-center gap-2 sm:gap-4 min-w-0">
            <Link to="/owner/dashboard" className="flex items-center gap-2 sm:gap-2.5 group min-w-0">
              ...
              <div className="flex flex-col min-w-0">
                <span className="font-black text-brand-green text-base sm:text-lg tracking-tight mt-2 leading-none truncate uppercase">
                  LETSELLR
                </span>
                <span className="text-[9px] sm:text-[10px] font-extrabold uppercase tracking-wider -mt-1 text-black truncate">
                  {roleLabel} Workspace
                </span>
              </div>
            </Link>
          ...
          <div className="flex items-center gap-2 sm:gap-3 shrink-0">
            ...
            <Link
              ...
              className="flex items-center gap-1.5 sm:gap-2 bg-brand-green hover:bg-brand-green-hover text-white px-3 sm:px-4 py-1.5 sm:py-2 rounded-lg text-xs font-black shadow-xs transition-all cursor-pointer whitespace-nowrap"
            >
            ...
            <DropdownMenu>
              <DropdownMenuTrigger className="flex items-center gap-1.5 sm:gap-1 p-1 rounded-lg hover:bg-slate-100 transition-colors cursor-pointer focus:outline-none border-0 bg-transparent">
                <div className="h-8 w-8 sm:h-9 sm:w-9 rounded-full bg-slate-900 text-white font-black text-xs flex items-center justify-center shadow-xs border border-slate-200 shrink-0 overflow-hidden">
                  {profileImage ? (
                ...
                <div className="hidden lg:flex flex-col text-left pr-1 justify-center">
                  <span className="text-xs font-extrabold text-slate-900 line-clamp-1 max-w-30 leading-5.5">
                    {user?.name || "Partner User"}
                  </span>
                  <span className="text-[9px] font-bold text-brand-green uppercase tracking-wider -mt-0.5 leading-none">
                    {roleLabel}
                  </span>
                </div>
        ...
        {navLinks.map((link) => {
          ...
          return (
            <Link
              key={link.path}
              to={link.path}
              className={`flex flex-col items-center gap-1 px-4 py-1.5 rounded-lg text-[10px] font-black transition-all ${
                isActive 
